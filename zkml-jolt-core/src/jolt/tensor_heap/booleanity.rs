use ark_serialize_derive::{CanonicalDeserialize, CanonicalSerialize};
use jolt_core::{field::JoltField, poly::{eq_poly::EqPolynomial, multilinear_polynomial::{BindingOrder, MultilinearPolynomial}}, subprotocols::sumcheck::{BatchableSumcheckInstance, SumcheckInstanceProof}, utils::{thread::unsafe_allocate_zero_vec, transcript::Transcript}};
...
struct BooleanitySumcheck<F: JoltField> {
    ...
    trace: Option<Vec<JoltONNXCycle>>,
}
...
pub fn prove_ra_booleanity<F: JoltField, ProofTranscript: Transcript>(
    trace: &[JoltONNXCycle],
    ...
    for (cycle_idx, cycle) in trace.iter().enumerate() {
        ...
        let mut remaining_address = ra[cycle_idx];
        for i in 0..d {
